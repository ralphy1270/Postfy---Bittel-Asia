...
class Message {
	...
	public function getMessages($otherUser) {
		...
		while($row = mysqli_fetch_array($get_messages_query)) {
			...
			$button = "<span onclick='deleteMessage($id, this)' style='background-color:rgba(231, 76, 60,1.0); color :white; padding:0 3px 0 3px; margin-right: 10px; border-radius: 4px; cursor: pointer;'>X</span>";
	...
	public function getConvosDropdown($data, $limit) {
		...
		foreach($convos as $username) {
			...
			$is_unread_query = mysqli_query($this->con, "SELECT body FROM messages WHERE user_to='$userLoggedIn' AND user_from='$username' ORDER BY id DESC");
